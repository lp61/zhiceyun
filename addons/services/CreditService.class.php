...
class CreditService extends Service {
    ...
    public function setUserCredit($uid, $action, $type=1) {
        ...
        if($type == 'reset') {
            foreach($this->creditType as $v){
                $creditUserLog[$v['name']]  = $creditUser[$v['name']] = $creditSet[$v['name']];
            }
        }else{
            $type = intval($type);
            foreach($this->creditType as $k => $v){
                $creditUser[$v['name']]    = $creditUser[$v['name']] + ( $type * $creditSet[$v['name']] );
                $creditUserLog[$v['name']] = $type * $creditSet[$v['name']];
            }
        }
        ...
        if ($res && !empty($creditUserLog)) { // 积分日志
            $creditUserLog['uid']          = $uid;
            $creditUserLog['credit_name']  = $action;
            $creditUserLog['credit_alias'] = $creditSet['alias'];
            $creditUserLog['ctime']        = time();
            M('credit_user_log')->add($creditUserLog);

            if (isset($creditUserLog['score'])) {
                $msg['title'] = '积分变动';
                $msg['content'] = $creditSet['alias'] . '，积分'.($creditUserLog['score']>0 ? '增加':'减少').abs($creditUserLog['score']).'。';
                model('AppMessage')->postMessage($uid, $msg);
                ...
                if ($touser) {
                    $score = $creditUser['score'];
                    $sendInfo['first'] = '亲爱的开发者，您刚刚'.($creditUserLog['score']>0 ? '获得':'消耗').abs($creditUserLog['score']).'积分，尚余'.$score.'积分。';
                    $sendInfo['key1'] = M('user')->where("uid=$uid")->getField('email');
                    $sendInfo['key2'] = date("Y年m月d日 H:i");
                    $sendInfo['key3'] = $creditUserLog['score'];
                    $sendInfo['key4'] = $creditUser['score'];
                    $sendInfo['key5'] = $creditSet['alias'];
                    service('Weixin')->send_notice($touser, 4, $sendInfo);
                }
            }
        }
