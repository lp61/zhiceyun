...
class SystemPopedomService extends Service {
    ...
    public function getGidByNode($node){
        if( ($cache = F('Cache_Node')) === false){
            ...
            $cache  = M('')->query($sql);
            $cache  = $this->getRelateNodes($cache);
            F('Cache_Node',$cache);
        }
        $gid = array();
        foreach ($cache as $v){
    ...
    public function getRelateNodes($cache) {
        if (empty($cache)) {
            return array();
        }
        ...
        $cc       = array();
        $tmpCache = array();
        foreach ($cache as $key => $_one) {
            if ($_one['has_related']) {
                $cc[]                       = $_one['node_id'];
                $tmpCache[$_one['node_id']] = $_one;
            }
        }
        $prefix = C('DB_PREFIX');
        $sql    = "select  * from {$prefix}node where parent_node_id in (".implode(',', $cc).")";
        $data   = M('')->query($sql);
        foreach ($data as $k => $_one) {
            $_one['user_group_id'] = $tmpCache[$_one['parent_node_id']]['user_group_id'];
            $cache[] = $_one;
        }
        return $cache;
    }
