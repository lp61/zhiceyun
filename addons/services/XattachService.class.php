...
class XattachService extends Service {
    ...
    public function upload($attach_type = 'attach', $input_options = array()) {
        ...
        if (empty($system_default['attach_path_rule']) || empty($system_default['attach_max_size']) || empty($system_default['attach_allow_extension'])) {
            ...
            $data['attach_max_size'] = '500'; // 默认2M
            $data['attach_allow_extension'] = 'jpg,gif,png,jpeg,bmp,zip,rar,doc,xls,ppt,docx,xlsx,pptx,pdf,apk';
        ...
        if (!$upload->upload()) {
            ...
            if ($options['save_to_db']) {
                foreach ($upload_info as $u) {
                    ...
                    $apk_info = $this->getApkInfo($map['extension'], $map['savepath'], $map['savename']);
                    if ($apk_info === FALSE) {
                        $return['status'] = false;
                        $return['info'] = "APK文件异常";
                        return $return;
                    }
                    $map = array_merge($map, $apk_info);
                    $aid = $xattach->add($map);
                    $map['id'] = intval($aid);
                    $map['key'] = $u['key'];
                    $infos[] = $map;
     ...
     */
    public function getApkInfo($extension, $savepath, $savename) {
        if ($extension != 'apk') {
            return array();
        }
        $apkfile = UPLOAD_PATH . "/" . $savepath . $savename;
        //log::write(print_r($apkfile, TRUE), LOG::INFO);
        //读取apk信息到数组中
        $tmp_array = array();
        exec("aapt d badging $apkfile|egrep -i 'versionName|label|icon';du -s $apkfile ", $tmp_array);
        //log::write(print_r($tmp_array, TRUE), LOG::INFO);
        $start = strpos($tmp_array[1], "label='") + strlen("label='");
        $len = strpos($tmp_array[1], "' icon=") - $start;
        $map['apk_name'] = substr($tmp_array[1], $start, $len);
        $data['apk_name'] = $map['apk_name'] ? $map['apk_name'] : '';

        $start = strpos($tmp_array[1], "icon='") + strlen("icon='");
        $len = strrpos($tmp_array[1], "'") - $start;
        $map['apk_icon'] = substr($tmp_array[1], $start, $len);

        $start = strpos($tmp_array[0], "versionName='") + strlen("versionName='");
        $len = strrpos($tmp_array[0], "'") - $start;
        $map['apk_version'] = substr($tmp_array[0], $start, $len);
        ...
        //dump($map);
        if (!empty($data['apk_icon'])) {
            $icon_path = UPLOAD_PATH . "/" . $data['apk_icon']; //apk图片保存绝对路径
            $file = tempnam("/tmp", "zip"); // 生成临时文件
            file_put_contents($file, file_get_contents($apkfile));
            $zip = new ZipArchive;
            $zip->open($file, ZIPARCHIVE::CREATE);
            if(!$zip->getFromName('AndroidManifest.xml')){
                return FALSE;
            }
            file_put_contents($icon_path, $zip->getFromName($icon_apk_path));
            $zip->close();
            @unlink($file);
            $data['apk_icon'] = UPLOAD_URL . "/" . $data['apk_icon'];
        }
