...
class CheckCode {
    ...
    public static function isValid($str, $check_code_tag = null, $isDel = false) {
        ...
        $checkcode = isset($_SESSION[$name]) ? $_SESSION[$name] : '';
    ...
    public static function save($val, $check_code_tag = null) {
        if (!empty($val)) {
            $name            = self::_getKey ( $check_code_tag );
            $_SESSION[$name] = md5(strtoupper($val));
        }
    }
...
//                '#707070',  // 灰色
        ...
        $default = array(
            ...
            'background' => __CORE__ . '/util/check_code/captcha_new/backgrounds/'.$backgrounds[$background_key],
        );
