...
class AttachModel extends Model {
     ...
     * 如果id为数组,格式=》array(附件id) UPLOAD_URL.UPLOAD_URL_EXT. '/'.
    ...
    public function getAllAttachById($id){
        if(!is_array($id)){
            ...
            foreach($data as $value){
                $result[$value['id']] = $value['savepath'].$value['savename'];
            }
            return $result;
        }
    }
    /**
     * [setAllAttachUserIdById 设置用户id]
     * @param  [type] $id [description]
     * @return [type]     [description]
     */
    public function setAllAttachUserIdById($id, $userId, $attach_type="attach"){
        if (empty($userId)) {
            return false;
        }
        $data['userId']      = $userId;
    ...
    public function deleteAttach($ids, $with_file = false, $with_thumb = false) {
        ...
        if ($with_file) {
            foreach ($attaches as $v) {
                $path = SITE_PATH . UPLOAD_URL_EXT . $v['savepath'] . $v['savename'];
                if ( is_file($path) )
                    unlink($path);
                if($with_thumb){
                    $middle_path = SITE_PATH . UPLOAD_URL_EXT . $v['savepath'] . 'middle_' .$v['savename'];
                    $small_path  = SITE_PATH . UPLOAD_URL_EXT . $v['savepath'] . 'small_' .$v['savename'];
