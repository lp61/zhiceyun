...
//获取用户姓名
function getUserName($uid, $lang='zh', $isEmail=ture) {
    $nameType = $isEmail? 'email' : 'uname';
    global $ts;
    if ($uid == $ts['user']['uid'] || $uid == $ts['user']['uname'])
        return $ts['user'][$nameType];
    ...
    if(!isset($_MapName[$uid])){
        if(is_numeric($uid)){
            ...
            $userinfo = D('User', 'home')->getUserByIdentifier($uid, $nameType);
        }
        $_MapName[$uid] = $userinfo[$nameType];
    }
...
//返回解析的空间地址
function getUserSpace($uid, $class, $target, $text, $icon = true, $length=false) {
    ...
    if($t){
        ...
        elseif ($t['1']=='email') {
            $text = str_replace("{email}", $_userinfo[$uid]['email'], $text);
            $class  = ($class)  ? $class  : 'username';
        } else {
    ...
    if ($_userinfo[$uid]['domain'])
        $url = U('home/User/index', array('uid' => $_userinfo[$uid]['domain']));
    else
        $url = U('home/User/index', array('uid' => $uid));
    ...
    $user_space_info = "<a href='{$url}' class='{$class}' target='{$target}' title='{$textTmp}'>$text" . $user_name_end . "</a>";
...
function getUserInfo($uid, $uname, $mid, $status = false)
{
    ...
    $data['space']     = ($user['domain'])?U('home/User/index',array('uid'=>$user['domain'])):U('home/User/index',array('uid'=>$user['uid']));
