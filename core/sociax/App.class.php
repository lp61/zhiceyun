...
{//类定义开始
    ...
    static public function init()
    {
        ...
        if (APP_NAME == 'home' && MODULE_NAME == 'Index' && ACTION_NAME == 'index' && $_SESSION['wap_to_normal'] != '1' && cookie('wap_to_normal') != '1' && $_REQUEST['wap_to_normal'] != '1') {
    ...
    //[/RUNTIME]
    static public function cronBuild(){
        self::build();
    }
    /**
     +----------------------------------------------------------
    ...
    static protected function initGoDomain() {
        if ($_REQUEST['rp']) {

        }elseif (self::isAjax()) {
            
        }else{
            if ($_SERVER['HTTP_HOST'] == C('HOME_DOMAIN') && APP_NAME == 'admin') {
                $url = str_replace(C('HOME_DOMAIN'), C('ADMIN_DOMAIN'), U('admin/Index/index'));
            }else if ($_SERVER['HTTP_HOST'] == C('ADMIN_DOMAIN') && 
                (APP_NAME == 'home' && !in_array(ACTION_NAME, 
                    array('doAdminLogin', 'adminlogin', 'logoutAdmin', 'kissy', 'ajaxUpload','thumbImage')
                ))) {
                $url = str_replace(C('ADMIN_DOMAIN'), C('HOME_DOMAIN'), U('home/Index/index'));
            }
