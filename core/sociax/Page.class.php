...
{//类定义开始
    ...
    public function show($isArray=false){
		...
		$linkPage .= " <a class='current'><span class='current'>".$this->nowPage."</span></a>";
		if($rightPages>0){
        ...
        if( $this->nowPage <= $halfRoll || $this->totalPages <= $this->rollPage ){
            ...
            $theFirst = "<a href='".$url."&".C('VAR_PAGE')."=1' >1</a><a href='".$url."&".C('VAR_PAGE')."=2' >...</a>";
        }
		...
		if( ($this->totalPages-$this->nowPage) < $halfRoll || $this->totalPages <= $this->rollPage ){
            ...
            $theEnd = "<a href='".$url."&".C('VAR_PAGE')."=".($theEndRow-4)."' >...</a><a href='".$url."&".C('VAR_PAGE')."=$theEndRow' >{$theEndRow}</a>";
        }
