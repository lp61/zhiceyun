...
try {
    ...
    $obj->connect();
    //error_log(' $obj:'+$obj,3,'/app/updatalog.log');
    $file_dir = '/apktest/' . $uuid . "/"; //文件存储路径
    $file = $file_dir . str_replace(' ', '', $file_name); //文件完整地址
    //error_log(' '+$file,3,'/app/updatalog.log');
    $log = $obj->exists($file);
    ...
    if (!$log) { //文件不存在则写入此文件
        ...
        if ($log2) {
            ...
            //error_log(' $apkurl:'+$apkurl ,3,'/app/updatalog.log');
        ...
        echo '0';
        //error_log(' sql:'+$sql ,3,'/app/updatalog.log');
