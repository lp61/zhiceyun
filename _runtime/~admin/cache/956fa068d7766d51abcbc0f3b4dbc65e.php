...
<div class="so_main">
  ...
  <div id="searchUser_div" <?php if(($type)  !=  "searchUser"): ?>style="display:none;"<?php endif; ?>>
	...
	<div class="form2">
	<form method="post" action="<?php echo U('admin/User/doSearchUser');?>" id="J_form">
    <dl class="lineD">
      <dt>Email：</dt>
      <dd>
    ...
    <dl class="lineD">
      <dt>新旧用户：</dt>
      <dd>
      	<input name="status" type="radio" value="" <?php if(($status)  ==  ""): ?>checked<?php endif; ?>>全部
        <input name="status" type="radio" value="-1" <?php if(($status)  ==  "-1"): ?>checked<?php endif; ?>>新用户
        <input name="status" type="radio" value="0" <?php if(($status)  ==  "0"): ?>checked<?php endif; ?>>旧用户
      </dd>
    </dl>
  ...
  <div class="Toolbar_inbox">
	...
	<a href="javascript:void(0);" class="btn_a" onclick="sendEmail();">
        <span class="search_action">&nbsp;&nbsp;邮件群发&nbsp;&nbsp;</span>
    </a>
	<a href="<?php echo U('admin/User/saveExcel', array('queryCode'=>$queryCode));?>" class="btn_a"><span>导出Excel</span></a>
  </div>
  <div class="list">
  ...
  <tr>
    ...
    <th class="line_l" style="width:8%;">ID</th>
    <th class="line_l" style="width:23%;">用户信息</th>
    <?php /*<th class="line_l">部门信息</th>*/ ?>
    <th class="line_l" style="width:14%;">用户组</th>
    <th class="line_l" style="width:11%;">用户类型</th>
    <th class="line_l" style="width:14%;">注册时间</th>
    <th class="line_l" style="width:10%;">状态</th>
    <th class="line_l" style="width:15%;">操作</th>
  </tr>
  <?php if(is_array($data)): ?><?php $i = 0;?><?php $__LIST__ = $data?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><tr overstyle='on' id="user_<?php echo ($vo['uid']); ?>">
	    ...
	    <td>
			<?php if($vo['user_type']==1){ ?>vip用户<?php } ?>
			<?php if($vo['user_type']==2){ ?>企业用户<?php } ?>
			<?php if($vo['user_type']==0){ ?>普通用户<?php } ?>
		</td>
	    <td><?php echo (date("Y-m-d H:i",$vo["ctime"])); ?></td>
	    <td><?php if(($vo['is_active'])  ==  "0"): ?>未激活<?php else: ?>激活<?php endif; ?></td>
	    <td>
...
<script>
	...
	function sendEmail(uid) {
		...
		if(uid == '' || !confirm('群发邮件，确认继续？')) return false;
		
		var url = "<?php echo U('admin/Log/addSendEmail');?>";

		window.location.href = url+'&uids='+uid.toString();
	}
