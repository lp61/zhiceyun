define(function(require, exports, module){
    ...
    function ValidatorElement(fieldName) {
        ...
        {
            if (this.el.size() > 1) {
                alert("Name属性["+this.fieldName+"]相同的多个表单字段，只支持检测是否为空。不支持'"+func+"()'");
                return false;
            }
            return true;
        };
        ...
        this.setFocus = function(showFocusMsg)
        {
            ...
            if (this.type == 'hidden'){
                var $el = this.el;
                $($el[0]).after(ValidatorForm.focusTmpInput);
                ValidatorForm.focusTmpInput.show().focus().hide().blur();
            }
            return this;
        };
    ...
    var FormValidator = {
        ...
        getFormId        : function(formObj)
        {
            ...
            if (!formId) {
                ...
                $('[name]', formObj).each(function(fieldK, fieldV){
                    jFieldV = $(fieldV);
                    tagN    = jFieldV[0].tagName.toLowerCase();
        ...
        focusHandler : function(vObj)
        {
            ...
            var val = vObj.el.val();
            if ((vObj.type == 'text' || vObj.el[0].tagName.toLowerCase()=='textarea') && (val == '' || val == vObj.emptyValue || val == vObj.defaultDbValue))
            {
                if (vObj.focusValue != '') {
        ...
        blurHandler : function(vObj)
        {
            ...
            if ( vObj.emptyValue != '' && (vObj.type == 'text' || vObj.el[0].tagName.toLowerCase()=='textarea') && (val == '' || val == vObj.focusValue || val == vObj.defaultDbValue))
            {
