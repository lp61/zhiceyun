...
$.fn.Huploadify = function(opts){
	...
	var defaults = {
		...
		fileSizeLimit:500000000,//允许上传的文件大小，单位KB
		  ...
		  funSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					...
					if(option.showUploadedSize){
						var num = '<span class="progressnum"><span class="uploadedsize">'+initFileSize+'</span><span>/</span><span class="totalsize">${fileSize}</span></span>'.replace(/\${fileSize}/g,formatFileSize(file.size));
