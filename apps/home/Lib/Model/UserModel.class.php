...
class UserModel extends Model {
    const CACHE_KEY_PREFIX  = 'SS_userInfo_';
    ...
    private function upbase( ){
        ...
        //修改登录用户缓存信息--名称
        $userLoginInfo = S(self::CACHE_KEY_PREFIX.$this->uid);
        if(!empty($userLoginInfo)) {
            $userLoginInfo['uname'] = $data['uname'];
            S(self::CACHE_KEY_PREFIX.$this->uid, $userLoginInfo);
        }
    ...
    public function getUserInfoCache($uid, $type = 'uid') {
        ...
        $userInfo = S(self::CACHE_KEY_PREFIX.$uid);
        //获取用户基本信息缓存
        if(empty($userInfo)) {
            //姓名
            ...
            S(self::CACHE_KEY_PREFIX.$uid, $userInfo);
        }
        return $userInfo;
    }
    /**
     * [resetUserInfoCache 清除用户信息缓存]
     * @param  [type] $uid [description]
     * @return [type]      [description]
     */
    public function resetUserInfoCache($uid) {
        if ($uid) {
            S(self::CACHE_KEY_PREFIX.$uid, null);
        }
    }
}