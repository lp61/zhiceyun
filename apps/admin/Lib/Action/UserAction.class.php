...
class UserAction extends AdministratorAction {
    ...
    public function doEditUser() {
        ...
        // D('User', 'home')->resetUserInfoCache($_POST['uid']);
    ...
    public function doSearchUser() {
        ...
        //组装搜索条件
        $fields = array('email','uid','sex','is_active','status');
        $map    = array();
        foreach($fields as $v)
        ...
        $queryCode = base64_encode(serialize($map));
    ...
    public function saveExcel() {
        ...
        //判断是否有搜索条件
        if(!empty($map)){
             $userList = D('User', 'home')->getUserList($map, true, true, '*', 'uid ASC', 2000);
        }else{
             $userList = D('User', 'home')->getUserList($map, true, true, '*', 'uid ASC', 30);
        }     
        ...
        $n = 2;
        foreach((array)$userList['data'] as $key =>$val){
            $user_group = '暂无用户组信息';
            if ($val['user_group']) {
                foreach ($val['user_group'] as $k => $v) {
                    $user_group_arr[] = $v['user_group_title'];
                }
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ->setCellValue('A'.$n, ' '. $val['uid'])
                ->setCellValue('B'.$n, ' '. $val['email'])
                ->setCellValue('C'.$n, ' '. $user_group)
                ->setCellValue('D'.$n, ' '. $user_type)
                ->setCellValue('E'.$n, ' '. date('Y-m-d H:i', $val['ctime']))
                ->setCellValue('F'.$n, ' '. $is_active_str);
            $n ++;
        }
    ...
    public function doAddNode($old_nid = 0) {
        ...
        $_POST['has_related']    = !empty($_POST['subAction']) ? 1:0;
        unset($_POST['node_id']);
