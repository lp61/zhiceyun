...
class HadoopAction extends AdministratorAction {
     ...
     * Hadoop文件管理
     */
    public function hadoopInfo($map){
        $obj = new phdfs("10.1.10.101", "9000");
        $hdfsPath = "hdfs://10.1.10.101:9000";
        $obj->connect();
        $apktestList = $obj->list_directory("/apktest/000564f1-86b6-0d04-0f6d-8b489336edb2/");
        $apktestinfo = $obj->file_info("/apktest/");
        $allList = $obj->list_directory("/");
        $data =array();


        $tree = self::toLayer($allList);

//        $level++;//子栏目层级
//        foreach ($allList as $key => $_one) {
//            $data[$key] = $_one;
//            if ($_one[size] == 0){
//                $data[$key][id] = $key;
//                $data[$key][fileType] = "文件夹";
//                $data[$key][package] = ture;
//                $path = "http://42.96.171.6:8008/zhiceyun/file/download.php?file=streamFile".substr($data[$key][name],strlen($hdfsPath));
//                $data[$key][path] = $path;
//
//                $dataList = $obj->list_directory(substr($data[$key][name],strlen($hdfsPath)));
//                foreach ($dataList as $key => $_one) {
//                    if()
//                }
//            }
//        }
        ...
        $this->assign($_POST);
        $this->assign('isSearch', empty($map)?'0':'1');
        $this->display(hadoopInfo);
    }

    static public function toLayer($cate, $name = 'child', $path = 0){
        $obj = new phdfs("10.1.10.101", "9000");
        if($path!=0){
        $hdfsPath = "hdfs://10.1.10.101:9000";
            $cate = $obj->list_directory($path);
        }
        $arr = array();
        foreach ($cate as $v) {
            if ($v['size'] == $pid) {
                $v[$name] = self::toLayer($cate, $name, substr($v[name],strlen($hdfsPath)));
                $arr[] = $v;
            }
        }
    ...
    function arrToTree($data,$pathHadoop){
        $tree = array();
        foreach($data as $k => $v){
            if($v['size'] == 0){
    ...
    function setData($data,$key,$_one){
        $data[$key] = $_one;
        if ($_one[size] == 0) {
            $data[$key][id] = $key;
            $data[$key][fileType] = "文件夹";
            ...
            arrToTree($data,substr($data[$key][name], strlen($hdfsPath)));
        } else {
            $data[$key] = $_one;
            $data[$key][id] = $key;
            $data[$key][fileType] = "文件";
            $data[$key][package] = false;
            $path = "http://42.96.171.6:8008/zhiceyun/file/download.php?file=streamFile" . substr($data[$key][name], strlen($hdfsPath));
            $data[$key][path] = $path;
        }
        return $data;
